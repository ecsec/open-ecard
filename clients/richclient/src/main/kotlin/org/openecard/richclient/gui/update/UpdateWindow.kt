 ...
 */
class UpdateWindow(
	private val updateChecker: VersionUpdateChecker,
	...
	private val elements: List<Node?>
		get() {
			...
			val versionCol =
				TableColumn<VersionUpdateTableItem, Hyperlink>(I18N.strings.update_version.localized())
			versionCol.cellValueFactory = PropertyValueFactory("version")
			val updateTypeCol =
				TableColumn<VersionUpdateTableItem, Hyperlink>(I18N.strings.update_type.localized())
			updateTypeCol.cellValueFactory = PropertyValueFactory("updateType")
			val downloadLinkCol =
				TableColumn<VersionUpdateTableItem, Hyperlink>(I18N.strings.update_direct_download.localized())
			downloadLinkCol.cellValueFactory = PropertyValueFactory("downloadLink")
			...
			table.setRowFactory(
				Callback { p: TableView<VersionUpdateTableItem>? ->
			...
			updates?.let { (data, advice) ->
				...
				val type =
					when (advice) {
						UpdateAdvice.MAINTAINED_UPDATE -> I18N.strings.update_version_minor.localized()
						UpdateAdvice.UPDATE -> I18N.strings.update_version_minor.localized()
						UpdateAdvice.SECURITY_UPDATE -> I18N.strings.update_version_security.localized()
						else -> I18N.strings.update_version_major.localized()
					}
			...
			table.columns.forEach(
				Consumer { column: TableColumn<VersionUpdateTableItem?, *> -> column.setSortable(false) },
			)
			...
			val label =
				if (updates != null) {
					if (updates.second == UpdateAdvice.UNMAINTAINED) {
						Label(
							I18N.strings.update_version_not_maintained
			...
			val vbox = VBox()
			val labelPage = Label(I18N.strings.update_manual_download.localized())
