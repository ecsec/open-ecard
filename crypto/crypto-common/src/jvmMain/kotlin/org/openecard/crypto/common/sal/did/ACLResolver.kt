 ...
 * @author Tobias Wich
 */
class ACLResolver(private val dispatcher: Dispatcher, handle: ConnectionHandleType) {
    ...
    @Throws(WSHelper.WSException::class, SecurityConditionUnsatisfiable::class)
    fun getUnsatisfiedDIDs(target: TargetNameType, acls: MutableList<AccessRuleType>): List<DIDStructureType> {
        val dids = getMissingDids(acls, target)
