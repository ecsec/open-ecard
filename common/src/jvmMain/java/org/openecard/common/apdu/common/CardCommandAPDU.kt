...
open class CardCommandAPDU : CardAPDU {
				...
				if (tmp == 0) {
					...
					if (bais.available() < 3) {
						...
						if (bais.available() == 1) {
							...
							require(bais.available() <= 3) { "Malformed APDU." }
						}
					}
				} else if (tmp > 0) {
					// Case 3, 4, 4.2
	...
	fun toByteArray(): ByteArray {
		...
		try {
			...
			LOG.error(ex) { "Failed to create APDU in memory." }
		}
	...
	fun transmit(
		...
		slotHandle: ByteArray?,
	): CardResponseAPDU = transmit(dispatcher, slotHandle, CardCommandStatus.responseOk())
