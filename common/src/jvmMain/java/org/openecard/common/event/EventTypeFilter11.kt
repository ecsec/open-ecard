 ...
 * @author Tobias Wich
 */
class EventTypeFilter(vararg eventType: EventType?) : EventFilter {
    private var eventType: ArrayList<EventType>? = null

    init {
        if (eventType.size == 0) {
            this.eventType = ArrayList(Arrays.asList(*EventType.entries.toTypedArray()))
        } else {
            this.eventType = ArrayList(Arrays.asList(*eventType))
        }
    }
