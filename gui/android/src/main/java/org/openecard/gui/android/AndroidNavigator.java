...
/**
 * Android base class for GUI Navigators.
 *
 * @author Tobias Wich
 */
public abstract class AndroidNavigator implements UserConsentNavigator {

    private static final Logger LOG = LoggerFactory.getLogger(AndroidNavigator.class);
	...
	ExecutorService executor = Executors.newFixedThreadPool(2, task -> {
	    return new Thread(task, String.format("AndroidGUI-%d", threadIdx.getAndIncrement()));
	});
	...
	try {
		...
		try {
		    ...
		    return new AndroidResult(stepObj, ResultStatus.CANCEL, Collections.emptyList());
		}
	    }
	} catch (InterruptedException ex) {
	    return new AndroidResult(stepObj, ResultStatus.INTERRUPTED, Collections.emptyList());
	} finally {
	...
	return () -> {
	    ...
	    return new AndroidResult(stepObj, r, Collections.emptyList(), sar.getReplacement());
	};
    }
