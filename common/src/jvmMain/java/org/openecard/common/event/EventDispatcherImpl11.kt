...
class EventDispatcherImpl : EventDispatcher {
    ...
    override fun add(cb: EventCallback, vararg eventTypes: EventType): EventCallback {
        add(cb, EventTypeFilter(*eventTypes))
        return cb
    }

    @Synchronized
    override fun add(cb: EventCallback, filter: EventFilter): EventCallback {
        if (initialized) {
    ...
    @Synchronized
    override fun notify(t: EventType, o: EventObject) {
        if (initialized) {
    ...
    companion object {
        private val LOG: Logger = LoggerFactory.getLogger(EventDispatcherImpl::class.java)
    }
}
