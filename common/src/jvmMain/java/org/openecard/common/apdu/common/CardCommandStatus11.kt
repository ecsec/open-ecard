...
object CardCommandStatus {
    ...
    fun getMessage(status: ByteArray): String {
        var msg = defaultMsg
        when (status[0]) {
            0x90.toByte() -> when (status[1]) {
                0x00 -> msg = "No error."
            }

            0x61 -> msg = "No error, but " + (0xFF and status[1].toInt()) + " bytes left to read."
            0x62 -> when (status[1]) {
                ...
                else -> if (status[1] >= 0x02 && status[1] <= 0x80) {
            ...
            0x63 -> when (status[1]) {
                ...
                else -> if (status[1] >= 0xC0 && status[1] <= 0xCF) {
            ...
            0x6B.toByte() -> when (status[1]) {
                0x00 -> msg = sw6B
            }

            0x6C.toByte() -> msg = sw6C + " " + (0xFF and status[1].toInt()) + " bytes available."
            0x6D.toByte() -> when (status[1]) {
                0x00 -> msg = sw6D
            }

            0x6E.toByte() -> when (status[1]) {
                0x00 -> msg = sw6E
            }

            0x6F.toByte() -> when (status[1]) {
                0x00 -> msg = sw6F
            }
        }
    ...
    fun response(vararg expected: Int): List<ByteArray> {
        val conv = arrayOfNulls<ByteArray>(expected.size)
        for (i in expected.indices) {
            conv[i] = toByteArray(expected[i].toShort(), true)
        }
