 ...
 */
class PAOS(
    @Nonnull dispatcher: Dispatcher, @param:Nonnull private val tlsHandler: TlsConnectionHandler,
    @param:Nonnull private val schemaValidator: DocumentSchemaValidator
) {
    ...
    private val validationError: Promise<DocumentValidatorException?>
     ...
     */
    init {
        this.dispatcher = dispatcher.filter
        this.validationError = Promise<DocumentValidatorException?>()
    ...
    private fun getRelatesTo(msg: SOAPMessage): String? {
        return getHeaderElement(msg, RELATES_TO)
    }

    @Throws(SOAPException::class)
    private fun setRelatesTo(msg: SOAPMessage, value: String?) {
        val elem = getHeaderElement(msg, RELATES_TO, true)
        elem.setTextContent(value)
    }

    @Throws(SOAPException::class)
    private fun getHeaderElement(msg: SOAPMessage, elem: QName): String? {
        val headerElem = getHeaderElement(msg, elem, false)
        return if (headerElem == null) null else headerElem.getTextContent().trim { it <= ' ' }
    }

    @Throws(SOAPException::class)
    private fun getHeaderElement(msg: SOAPMessage, elem: QName, create: Boolean): Element {
        var result: Element? = null
        val h = msg.soapHeader
        // try to find a header
        for (e in h.childElements) {
            if (e.getLocalName() == elem.getLocalPart() && e.getNamespaceURI() == elem.getNamespaceURI()) {
                result = e
                break
            }
        }
    ...
    @Throws(SOAPException::class)
    private fun getMessageID(msg: SOAPMessage): String {
        return getHeaderElement(msg, MESSAGE_ID)!!
    }

    @Throws(SOAPException::class)
    private fun setMessageID(msg: SOAPMessage, value: String?) {
    ...
    private fun processPAOSRequest(content: InputStream): Any {
        try {
            ...
            if (body.getLocalName() == "StartPAOSResponse" && !body.hasAttribute("Profile")) {
            ...
            while (true) {
                ...
                try {
                    do {
                        ...
                        req.setHeader("Content-Length", reqMsg.getContentLength().toString())
...
//		throw new PAOSException(ex);
    ...
    private fun checkHTTPStatusCode(statusCode: Int) {
        ...
        if (statusCode != 200 && statusCode != 202) {
            ...
            val msg2 = ("The PAOS endpoint sent the http status code 200 which does not conform to the "
    ...
    private fun synthesizeObj(content: ByteArrayInputStream, cause: DocumentValidatorException): Any? {
        try {
            val doc = m.str2doc(content)
            val msg = m.doc2soap(doc)
    ...
    companion object {
        private val LOG: Logger = LoggerFactory.getLogger(PAOS::class.java)
