...
class DynamicContext private constructor() {
    ...
    @Nonnull
    @Synchronized
    fun getPromise(@Nonnull key: String): Promise<Any> {
        var p = context[key]
        if (p != null) {
            return p
        } else {
     ...
     */
    fun get(@Nonnull key: String): Any? {
     ...
     */
    @Synchronized
    fun put(@Nonnull key: String, value: Any?): Any? {
     ...
     */
    @Synchronized
    fun putPromise(@Nonnull key: String, @Nonnull p: Promise<*>?) {
     ...
     * @return The previous value for key or `null` if there was no previous mapping or `null` was mapped.
     */
    fun remove(@Nonnull key: String): Any? {
    ...
    companion object {
        private val LOG: Logger = LoggerFactory.getLogger(DynamicContext::class.java)

        private var LOCAL_MAP: InheritableThreadLocal<Map<String, DynamicContext>>

        init {
            LOCAL_MAP = object : InheritableThreadLocal<Map<String?, DynamicContext?>?>() {
                override fun initialValue(): Map<String?, DynamicContext?>? {
                    return ConcurrentSkipListMap()
                }

                override fun childValue(parentValue: Map<String?, DynamicContext?>?): Map<String?, DynamicContext?>? {
                    return parentValue
                }
            }
        }
        ...
        @JvmStatic
        @Nonnull
        fun getInstance(@Nonnull key: String): DynamicContext? {
