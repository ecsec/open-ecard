 ...
 * @author Tobias Wich
 */
class IFDStatusDiff(private val expected: List<IFDStatusType>) {
    private var result: MutableList<IFDStatusType>? = null
    private var deleted: MutableList<IFDStatusType?>? = null


    fun diff(others: List<IFDStatusType>, withNew: Boolean) {
        result = LinkedList()
    ...
    fun hasChanges(): Boolean {
        return if (this.result != null && !result!!.isEmpty()) {
            true
        } else {
            false
        }
    ...
    fun result(): List<IFDStatusType>? {
        return result
    }

    companion object {
        private val _logger: Logger = LoggerFactory.getLogger(IFDStatusDiff::class.java)

        fun diff(expected: List<IFDStatusType>, others: List<IFDStatusType>, withNew: Boolean): IFDStatusDiff {
            val diff = IFDStatusDiff(expected)
            diff.diff(others, withNew)
            return diff
        }

        private fun arrayEquals(a: ByteArray?, b: ByteArray?): Boolean {
            if (a == null && b == null) {
                return false
            } else {
