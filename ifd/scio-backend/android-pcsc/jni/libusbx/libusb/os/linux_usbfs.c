 ...
 *
 * Modified 2013 Dirk Petrautzki <petrautzki@hs-coburg.de>
 *  - Use socket communication to retrieve the file descriptor from
 *    Android instead of opening the file oneself
 *
...
const char* fdSockaddr = "/data/data/org.openecard.android/files/socket";
const char* pathSockaddr = "/data/data/org.openecard.android/files/socket2";
...
/* Receive a file descriptor from the socket given through parameter fd */
int recv_fd(int fd)
{
   int             newfd, nr, status;
   char            *ptr;
   ...
   for ( ; ; ) {
       ...
       if ((nr = recvmsg(fd, &msg, 0)) < 0) {
           fprintf(stderr, "UNIX recvmsg error");
           exit(1);
       } else if (nr == 0) {
...
/* Receive a file descriptor from the socket located at sockaddr */
int get_from_client(char * sockaddr) {
    LOGI("Getting FD from socket: %s\n", sockaddr);
    int s, t, len;
    ...
    LOGI("Got FD recieve socket");
    remote.sun_family = AF_UNIX;
    strcpy(remote.sun_path, sockaddr);
    len = strlen(remote.sun_path) + sizeof(remote.sun_family);
    while (connect(s, (struct sockaddr *)&remote, len) == -1) {
        usleep(10*1000); // 10 milliseconds
        //LOGI("waiting for connet on fd recv socket");
    }
    LOGI("Connected to FD recieve socket");
    sock = recv_fd(s);
    LOGI("Recieved FD from socket: %d\n", sock);
    close(s);

    return sock;
}

void sendPath(char * sockaddr, char * path) {
	int done, n, len, ret;
	unsigned int pathSocket,  pathSocket2, t;
	struct sockaddr_un local, remote;
	LOGI("sending path: %s\n", sockaddr);

	pathSocket = socket(AF_UNIX, SOCK_STREAM, 0);
	if (pathSocket < 0) {
		LOGI("UNIX Socket returned an error %s\n", strerror(errno));
		return;
	}
	LOGI("Got path sending socket\n");

	local.sun_family = AF_UNIX;  /* local is declared before socket() ^ */
	strcpy(local.sun_path, sockaddr);
	unlink(local.sun_path);
	len = strlen(local.sun_path) + sizeof(local.sun_family);

	if (bind(pathSocket, (struct sockaddr *)&local, len) == -1) {
		LOGI("UNIX Bind error: %s", strerror(errno));
		return;
	}
	LOGI("Binding successfull\n");

	if (listen(pathSocket, 1) == -1) {
		LOGI("UNIX Listen error: %s\n", strerror(errno));
		return;
	}
	LOGI("listen successfull, waiting for accept\n");

	t = sizeof(remote);
	if ((pathSocket2 = accept(pathSocket, (struct sockaddr *)&remote, &t)) == -1) {
		LOGI("UNIX Accept error");
		return;
	}
	LOGI("connected, starting to send path: %s", path);

	ret = send(pathSocket2, path, strlen(path), 0);
	LOGI("return of path sending %d\n", ret);
	close(pathSocket2);
	close(pathSocket);
}

/* Send the device path and receive the corresponding file descriptor */
int getFD(char * filename){
	sendPath(pathSockaddr, filename);
	return get_from_client(fdSockaddr);
}
