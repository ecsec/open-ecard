...
import org.openecard.bouncycastle.asn1.x500.AttributeTypeAndValue;
...
import org.openecard.bouncycastle.asn1.x509.PolicyInformation;
...
public class ListCertificates {
	...
	public ListCertificates(TokenCache tokenCache, byte[] slotHandle, List<CertificateFilterType> certFilter,
		...
		CertificateFilterType f1 = new CertificateFilterType();
		f1.setKeyUsage(KeyUsageType.AUTHENTICATION);
		this.certFilter = Arrays.asList(f1);
		this.pin = pin;
	}
	...
	public List<CertificateInfoType> getCertificates() throws WSHelper.WSException, NoSuchDid, CertificateException,
		...
		try {
			...
			for (CertificateInfoType certInfo : result) {
				if (certInfo.getDIDName().equals("Authenticate-rsaSSA-PSS")) {
					return new ArrayList<>(Collections.singleton(certInfo));
				}
			}
