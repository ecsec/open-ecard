 ...
 */
object SysUtils {
    private val LOG: Logger = LoggerFactory.getLogger(SysUtils::class.java)
     ...
     */
    @Nonnull
    fun expandSysProps(@Nonnull s: String): String {
        // expand and clear unset vars
        var s = s
     ...
     */
    @Nonnull
    fun expandEnvVarsAndSysProps(@Nonnull s: String): String {
    ...
    private fun expandVars(s: String, env: Map<out Any, Any>, p: Pattern, clear: Boolean): String {
        var s = s
        val m = p.matcher(s)
        ...
        while (m.find()) {
            ...
            val replacement = envValueObj?.toString()
                ?: if (clear) {
                    ""
                } else {
                    null
    ...
    private fun removeQuote(s: String, p: Pattern): String {
        var s = s
        val m = p.matcher(s)
        s = m.replaceAll("$2")
        return s
