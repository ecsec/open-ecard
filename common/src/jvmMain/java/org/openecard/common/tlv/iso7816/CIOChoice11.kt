 ...
 * @author Tobias Wich
 */
class CIOChoice(tlv: TLV) : TLV(tlv) {
    ...
    init {
        ...
        if (p.match(Tag(TagClass.CONTEXT, false, 0))) {
            privateKeys = GenericPathOrObjects<Any?>(p.next(0)!!, PrivateKeyChoice::class.java)
            element = privateKeys
            ...
            certificates = GenericPathOrObjects<Any?>(p.next(0)!!, CertificateChoice::class.java)
            ...
            authObjects = GenericPathOrObjects<Any?>(p.next(0)!!, AuthenticationObjectChoice::class.java)
            element = authObjects
