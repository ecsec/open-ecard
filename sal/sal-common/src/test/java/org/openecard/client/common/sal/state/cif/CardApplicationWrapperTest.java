...
public class CardApplicationWrapperTest {
	...
	assertEquals(cardApplicationWrapper.getApplicationIdentifier(), rootApplication);
	assertEquals(cardApplicationWrapper.getDataSetNameList().getDataSetName().size(), 5);
	assertEquals(cardApplicationWrapper.getDIDInfoList().size(), 8);
	assertEquals(cardApplicationWrapper.getCardApplicationACL().getAccessRule().size(), 40);
	assertNotNull(cardApplicationWrapper
		.getSecurityCondition(CardApplicationServiceActionName.CARD_APPLICATION_LIST));
