...
class CIAInfo(tlv: TLV) : TLV(tlv) {
    ...
    init {
        if (tlv.tagNumWithClass != Tag.Companion.SEQUENCE_TAG.getTagNumWithClass()) {
        ...
        if (p.match(Tag(TagClass.CONTEXT, false, 2))) {
            supportedAlgorithms =
                TLVList(p.next(0)!!.child!!, Tag.Companion.SEQUENCE_TAG.getTagNumWithClass()).getContent()
        }
        // issuerId
        issuerId = null
