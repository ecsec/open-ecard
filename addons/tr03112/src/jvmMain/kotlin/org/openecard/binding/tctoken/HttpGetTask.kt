...
class HttpGetTask(
    ...
    override fun call(): StartPAOSResponse {
        ...
        val response: StartPAOSResponse = WSHelper.makeResponse<Class<StartPAOSResponse?>, StartPAOSResponse>(
    ...
    private val usedHandle: ConnectionHandleType?
        get() {
            if (credentialFac != null) {
                return credentialFac.usedHandle
            } else {
    ...
    private fun makeSmartcardCredentialFactory(): BaseSmartCardCredentialFactory {
        if (connectionHandle.getSlotHandle() == null) {
            val cf = SearchingSmartCardCredentialFactory(
                dispatcher,
                true,
                evtDispatcher,
                connectionHandle,
                HashSet<Any?>(tokenRequest.getTCToken().getAllowedCardType())
            )
            return cf
        } else {
            val cf = PreselectedSmartCardCredentialFactory(
                dispatcher,
                connectionHandle,
                true
            )
