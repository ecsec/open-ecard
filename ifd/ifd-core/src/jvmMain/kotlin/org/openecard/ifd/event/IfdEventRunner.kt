...
class IfdEventRunner(
	...
	private fun getCorresponding(
		ifdName: String,
		statuses: List<IFDStatusType>,
	): IFDStatusType? {
	...
	private fun fireEvents(diff: List<IFDStatusType>) {
		for (term in diff) {
			...
			for (slot in term.getSlotStatus()) {
				...
				LOG.debug {
					"Slot status: [terminalAdded: $terminalAdded, cardPresent: $cardPresent, cardWasPresent: $cardWasPresent]."
				}
