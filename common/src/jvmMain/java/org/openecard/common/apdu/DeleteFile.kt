 ...
 */
open class DeleteFile(
	p1: Byte,
	p2: Byte,
) : CardCommandAPDU(x00, INS_DELETE_FILE, p1, p2) {
	fun setFCI() {
		p2 = FCI
	}
	...
	class File(
		fid: ByteArray?,
	) : DeleteFile(0x00.toByte(), 0x0C.toByte()) {
		/**
	...
	class ChildDirectory(
		fid: ByteArray?,
	) : DeleteFile(0x01.toByte(), 0x0C.toByte()) {
		/**
	...
	class ChildFile(
		fid: ByteArray?,
	) : DeleteFile(0x02.toByte(), 0x0C.toByte()) {
		/**
		 * Creates a new DELETE FILE APDU to delete a file.
		 *
		 * @param fid File Identifier
		 ...
		 */
		constructor(fid: Short) : this(ShortUtils.toByteArray(fid))
	}
	...
	class Application(
		aid: ByteArray?,
	) : DeleteFile(0x04.toByte(), 0x0C.toByte()) {
		/**
	...
	class AbsolutePath(
		aid: ByteArray?,
	) : DeleteFile(0x08.toByte(), 0x0C.toByte()) {
		/**
	...
	class RelativePath(
		aid: ByteArray?,
	) : DeleteFile(0x09.toByte(), 0x0C.toByte()) {
		/**
