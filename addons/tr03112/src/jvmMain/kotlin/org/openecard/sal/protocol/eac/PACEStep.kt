 ...
 */
class PACEStep(ctx: Context) : ProtocolStep<DIDAuthenticate?, DIDAuthenticateResponse?> {
    ...
    override fun perform(request: DIDAuthenticate, internalData: MutableMap<String?, Any?>): DIDAuthenticateResponse {
        ...
        try {
            ...
            val eac1Output = eac1Input.getOutputType()

            val aad = AuthenticatedAuxiliaryData(eac1Input.getAuthenticatedAuxiliaryData())
            var pwId = PasswordID.PIN


            // Certificate chain
            val certChain = CardVerifiableCertificateChain(eac1Input.getCertificates())
            val rawCertificateDescription = eac1Input.getCertificateDescription()
            ...
            val requiredCHAT = CHAT(eac1Input.getRequiredCHAT())
            val optionalCHAT = CHAT(eac1Input.getOptionalCHAT())
            ...
            if (taCHAT.role != CHAT.Role.AUTHENTICATION_TERMINAL) {
                val msg = "Unsupported terminal type in Terminal Certificate referenced. Referenced terminal type is " +
            ...
            if (eacData.transactionInfo != null && eacData.transactionInfo.trim { it <= ' ' }.isEmpty()) {
            ...
            uc.getSteps().add(PINStep.Companion.createDummy(pinID))
                    ...
                    if (guiResult == ResultStatus.CANCEL || guiResult == ResultStatus.INTERRUPTED) {
                        ...
                        if (!paceErrorPromise.isDelivered()) {
                            ...
                            if (paceError is WSHelper.WSException) {
                                ...
                                when (minor) {
                                    ECardConstants.Minor.IFD.CANCELLATION_BY_USER, ECardConstants.Minor.SAL.CANCELLATION_BY_USER, ECardConstants.Minor.Disp.TIMEOUT -> needsTermination =
                                        true
                                }
                            }
            ...
            if (pPaceError != null) {
                ...
                if (pPaceError is WSHelper.WSException) {
                    ...
                    return response
                } else if (pPaceError is DispatcherException or pPaceError is java . lang . reflect . InvocationTargetException) {
            ...
            if (conHandle!!.getSlotHandle() == null) {
                throw RuntimeException("The connection handle stored in TR03112Keys.CONNECTION_HANDLE does not have a slot handle!")
            }
            val ta = TerminalAuthentication(dispatcher, conHandle.getSlotHandle())
            val challenge = ta.getChallenge()
     ...
     */
    private fun performChecks(certDescription: CertificateDescription, dynCtx: DynamicContext): Result {
    ...
    private fun checkTCTokenAndSubjectURL(certDescription: CertificateDescription, dynCtx: DynamicContext): Boolean {
    ...
    private fun checkEidServerCertificate(certDescription: CertificateDescription, dynCtx: DynamicContext): Boolean {
        val sameChannel = dynCtx.get(TR03112Keys.SAME_CHANNEL) as Boolean?
        if (Boolean.TRUE == sameChannel) {
    ...
    private fun checkTCTokenServerCertificates(
        ...
        val certificates: MutableList<Pair<URL?, TlsServerCertificate?>?>?
        certificates =
            dynCtx.get(TR03112Keys.TCTOKEN_SERVER_CERTIFICATES) as MutableList<Pair<URL?, TlsServerCertificate?>?>?
        if (certificates != null) {
            for (cert in certificates) {
                if (cert is Pair<*, *>) {
                    val u: URL = cert.p1!!
                    val host =
                        u.getProtocol() + "://" + u.getHost() + (if (u.getPort() == -1) "" else (":" + u.getPort()))
                    val bcCert: TlsServerCertificate = cert.p2!!
    ...
    companion object {
        private val LOG: Logger = LoggerFactory.getLogger(PACEStep::class.java.getName())
