 ...
 */
object HTMLUtils {
    private val HTML_ESCAPES: Map<Char, String> = object : HashMap<Char?, String?>() {
        init {
            put('&', "&amp;")
            put('<', "&lt;")
            put('>', "&gt;")
    ...
    fun escapeHtml(input: String?): String? {
        ...
        for (i in 0..<input.length) {
            result.append(encodeChar(input[i], HTML_ESCAPES))
        }
    ...
    private fun encodeChar(c: Char, mapping: Map<Char, String>): String {
