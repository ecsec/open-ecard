 ...
 * ([StepActionResult]) to a result of the step ([org.openecard.richclient.processui.StepResult]) and return it to the caller
 * of the navigators function (e.g. [UserConsentNavigator.next]). In case the step is closed by the user, then
 * the background task is aborted and the usual procedure is followed.
 *
 * @author Tobias Wich
 */
@FunctionalInterface
interface BackgroundTask : Callable<StepActionResult?>
