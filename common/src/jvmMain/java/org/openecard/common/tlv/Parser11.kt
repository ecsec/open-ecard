 ...
 * @author Tobias Wich
 */
class Parser(private val tlv: TLV) {
     ...
     */
    fun match(vararg tags: Tag?): Boolean {
     ...
     * @return `true` when the next object has one of the given tags, `false` otherwise.
     */
    fun match(vararg tagsWithClass: Long): Boolean {
     ...
     */
    fun matchLA(i: Int, vararg tagsWithClass: Long): Boolean {
        val lookahead = LA(i)
     ...
     * @return `true` when the ith object has one of the given tags, `false` otherwise.
     */
    fun matchLA(i: Int, vararg tags: Tag?): Boolean {
        val lookahead = LA(i)
        return match(lookahead, convertTags(*tags))
    }

    private fun match(tlv: TLV?, tagsWithClass: LongArray): Boolean {
        if (tlv != null) {
