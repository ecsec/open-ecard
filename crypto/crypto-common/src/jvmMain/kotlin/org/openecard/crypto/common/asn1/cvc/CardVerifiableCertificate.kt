 ...
 */
class CardVerifiableCertificate(cvc: TLV) {
    /**
        ...
        try {
            ...
            for (item in bodyElements) {
                ...
                when (itemTag) {
                    ...
                    TAG_CAR -> car = PublicKeyReference(
                        bodyObject.findChildTags(TAG_CAR.toLong())[0].value
                    )

                    TAG_PUBLIC_KEY -> publicKey = PublicKey.Companion.getInstance(
						bodyObject.findChildTags(TAG_PUBLIC_KEY.toLong())[0]
                    )

                    TAG_CHR -> chr = PublicKeyReference(
    ...
    private fun verify() {
        if (this.cPI == null || car == null || publicKey == null || chr == null || chat == null || effectiveDate == null || expirationDate == null) {
     ...
     */
    fun getExpirationDate(): Calendar {
     ...
     */
    fun compare(certificate: CardVerifiableCertificate): Boolean {
        return ByteUtils.compare(this.signature, certificate.signature)
    }
