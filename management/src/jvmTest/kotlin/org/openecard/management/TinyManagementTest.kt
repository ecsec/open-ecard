 ...
 */
class TinyManagementTest {
    private lateinit var env: ClientEnv
    private lateinit var instance: TinyManagement

    @BeforeMethod
    fun setUp() {
        env = ClientEnv()
        instance = TinyManagement(env)
    }


    @Test
    fun testInitializeFramework() {
        val initializeFramework = InitializeFramework()
        val initializeFrameworkResponse = instance.initializeFramework(initializeFramework)
        Assert.assertEquals(ECardConstants.Major.OK, initializeFrameworkResponse.getResult().getResultMajor())
        Assert.assertEquals(ECardConstants.ECARD_API_VERSION_MAJOR, initializeFrameworkResponse.getVersion().getMajor())
        Assert.assertEquals(ECardConstants.ECARD_API_VERSION_MINOR, initializeFrameworkResponse.getVersion().getMinor())
        Assert.assertEquals(
            ECardConstants.ECARD_API_VERSION_SUBMINOR,
            initializeFrameworkResponse.getVersion().getSubMinor()
        )
    }
    ...
    fun testRest() {
        for (m in instance.javaClass.getDeclaredMethods()) {
            // cobertura adds methods whose names start with '__cobertura'
            if (m.name == "initializeFramework" || m.name.startsWith("__cobertura")) {
                continue
            }
            println(m.name)

            if (m.parameterTypes.size == 1) {
                val o: Any = m.parameterTypes[0].getDeclaredConstructor().newInstance()

                val result = m.invoke(instance, o) as ResponseType
                Assert.assertEquals(ECardConstants.Major.ERROR, result.getResult().getResultMajor())
            }
        }
    }
    //    /**
