...
import org.openecard.richclient.gui.components.*
import java.awt.*
 ...
 */
open class SettingsGroup(title: String?, protected val properties: Settings) :
    JPanel() {
     ...
     */
    protected fun setEnabledComponent(element: Component, enabled: Boolean) {
    ...
    protected fun addInputItem(
        ...
        if (isPassword) {
            input = object : JPasswordField(value) {
    ...
    protected fun addBoolItem(
        ...
        if (value == null || value == "") {
            properties.setProperty(property, java.lang.Boolean.FALSE.toString())
        }
        ...
        fieldLabels.put(input, label)
        // add listener for value changes
        input.addItemListener(object : ItemListener {
    ...
    protected fun addMultiSelectionItem(
        ...
        for (value: String in values) {
            ...
            if (property2 != null) {
                val multProps: Array<String> = property2.split(";".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    private fun addLabel(name: String, description: String?): JLabel {
    ...
    private fun addListLabel(name: String, description: String?): JLabel {
        val label: JLabel = JLabel(name)
        label.setToolTipText(description)
        label.setFont(label.getFont().deriveFont(Font.PLAIN))
        val constraints: GridBagConstraints = GridBagConstraints()
    ...
    protected fun addScalarEntryTypNumber(
        ...
        val spinner: JSpinner = object : JSpinner(model) {
            override fun getPreferredSize(): Dimension {
                val dim: Dimension = super.getPreferredSize()
                dim.width = 100
                return dim
    ...
    protected fun addFileEntry(
        ...
        val filePathField: JTextField = object : JTextField() {
        ...
        filePathField.document.addDocumentListener(object : DocumentListener {
            override fun insertUpdate(e: DocumentEvent) {
