...
enum class SignatureAlgorithms(
	...
	val jcaAlg: String,
	val algId: String,
	val keyType: KeyTypes,
    val hashAlg: HashAlgorithms?
) {
    ...
    val isRsaPss: Boolean
        get() {
			return when (this) {
				CKM_SHA1_RSA_PKCS_PSS, CKM_SHA256_RSA_PKCS_PSS, CKM_SHA384_RSA_PKCS_PSS, CKM_SHA512_RSA_PKCS_PSS -> true
				else -> false
			}
        }

    val isRsaSsa: Boolean
        get() = this.keyType == KeyTypes.CKK_RSA && this.hashAlg != null && !this.isRsaPss

    companion object {
        private val mechanismLookup: HashMap<Long?, SignatureAlgorithms>
        private val jcaLookup: HashMap<String?, SignatureAlgorithms>
        private val algIdLookup: HashMap<String?, SignatureAlgorithms>

        init {
            val values = SignatureAlgorithms.entries.toTypedArray()

            mechanismLookup = HashMap<Long?, SignatureAlgorithms>(values.size)
            jcaLookup = HashMap<String?, SignatureAlgorithms>(values.size)
            algIdLookup = HashMap<String?, SignatureAlgorithms>(values.size)

            for (next in values) {
                mechanismLookup.put(next.pkcs11Mechanism, next)
        ...
        fun fromMechanismId(id: Long): SignatureAlgorithms {
            val result = mechanismLookup.get(id)
            if (result == null) {
                throw UnsupportedAlgorithmException(String.format("No mechanism defined for ID %08x.", id))
            } else {
                return result
            }
        }
        ...
        @Throws(UnsupportedAlgorithmException::class)
        fun fromJcaName(jcaName: String?): SignatureAlgorithms {
            val a = jcaLookup.get(jcaName)
            if (a == null) {
                throw UnsupportedAlgorithmException("No JCA Name $jcaName available.")
            } else {
                return a
            }
        }
        ...
        @Throws(UnsupportedAlgorithmException::class)
        fun fromAlgId(algId: String?): SignatureAlgorithms {
            val a = algIdLookup.get(algId)
            if (a == null) {
                throw UnsupportedAlgorithmException("No Algorithm ID $algId available.")
            } else {
                return a
            }
        }
		...
		@Throws(UnsupportedAlgorithmException::class)
        fun algIdtoJcaName(oid: String?): String? {
            val m = algIdLookup.get(oid)
            if (m == null) {
                throw UnsupportedAlgorithmException("No JCA Name available for OID $oid.")
            } else {
                return m.jcaAlg
            }
        }
        ...
        @Throws(UnsupportedAlgorithmException::class)
        fun jcaNameToAlgId(jcaAlgName: String?): String? {
            val m = jcaLookup.get(jcaAlgName)
            if (m == null) {
                throw UnsupportedAlgorithmException("No OID available for JCA Name $jcaAlgName.")
            } else {
                return m.algId
            }
        }
