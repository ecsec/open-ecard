...
open class Select(
	p1: Byte,
	p2: Byte,
) : CardCommandAPDU(x00, SELECT_INS, p1, p2) {
	/**
	...
	class File(
		fid: ByteArray?,
	) : Select(0x00.toByte(), NO_FILE_METADATA) {
		/**
	...
	class ChildDirectory(
		fid: ByteArray?,
	) : Select(0x01.toByte(), NO_FILE_METADATA) {
		/**
	...
	class ChildFile(
		fid: ByteArray?,
	) : Select(0x02.toByte(), NO_FILE_METADATA) {
		/**
		 * Creates a new Select APDU to select a file.
		 *
		 * @param fid File Identifier
	...
	class Application(
		aid: ByteArray?,
	) : Select(0x04.toByte(), NO_FILE_METADATA) {
		/**
	...
	class AbsolutePath(
		aid: ByteArray?,
	) : Select(0x08.toByte(), NO_FILE_METADATA) {
		/**
	...
	class RelativePath(
		aid: ByteArray?,
	) : Select(0x09.toByte(), NO_FILE_METADATA) {
		/**
