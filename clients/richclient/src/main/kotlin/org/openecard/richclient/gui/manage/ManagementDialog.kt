 ...
 * @author Tobias Wich
 */
class ManagementDialog(manager: AddonManager) : JFrame() {
    ...
    private fun createCoreList() {
        ...
        // add addon panels
        model.addElement(lang.translationForKey("addon.list.core.general"),
			createGeneralSettingsAddon()
		)
        model.addElement(lang.translationForKey("addon.list.core.connection"),
			createConnectionSettingsAddon()
		)
        model.addElement(lang.translationForKey("addon.list.core.logging"),
			createLogSettingsAddon()
		)
        model.addElement(lang.translationForKey("addon.list.core.middleware"),
    ...
    private inner class ClearSelectionListener(private val otherList: JList<*>?) : ListSelectionListener {
    ...
    private fun createAddonPaneFromAddonSpec(desc: AddonSpecification, model: AddonSelectionModel, coreAddon: Boolean) {
        ...
        if (desc.configDescription != null && desc.configDescription.entries.isNotEmpty()) {
            val group: DefaultSettingsGroup = DefaultSettingsGroup(
                lang.translationForKey("addon.settings.general"),
                settings, desc.configDescription.entries
            )
    ...
    companion object {
         ...
         */
        private fun loadLogo(loader: ClassLoader?, logoPath: String?): Image? {
