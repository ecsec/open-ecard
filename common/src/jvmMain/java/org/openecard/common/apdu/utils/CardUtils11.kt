...
import org.openecard.common.apdu.*
 ...
 */
object CardUtils {
    private val LOG: Logger = LoggerFactory.getLogger(CardUtils::class.java)
    ...
    fun selectMF(dispatcher: Dispatcher, slotHandle: ByteArray?) {
     ...
     */
    @Throws(APDUException::class)
    fun selectFile(dispatcher: Dispatcher?, slotHandle: ByteArray?, fileID: Short): CardResponseAPDU {
        return selectFile(dispatcher, slotHandle, toByteArray(fileID))
    }
     ...
     */
    @Throws(APDUException::class)
    fun selectFile(dispatcher: Dispatcher?, slotHandle: ByteArray?, fileID: ByteArray): CardResponseAPDU? {
    ...
    fun selectFileWithOptions(
        dispatcher: Dispatcher?, slotHandle: ByteArray?, fileIdOrPath: ByteArray,
        responses: MutableList<ByteArray?>?, resultType: FileControlParameters
    ): CardResponseAPDU? {
        ...
        while (i < fileIdOrPath.size) {
            if (fileIdOrPath[i] == 0x3F.toByte() && fileIdOrPath[i + 1] == 0x00.toByte() && i == 0) {
                ...
                selectFile = Select.ChildDirectory(
                    byteArrayOf(
                        fileIdOrPath[i],
                        fileIdOrPath[i + 1]
                    )
            ...
            if (responses == null) {
                ...
                if (!result.trailer.contentEquals(
                    ...
                    ) && i == 2 && fileIdOrPath[0] == 0x3F.toByte() && fileIdOrPath[1] == 0x00.toByte()
                ) {
     ...
     */
    @Throws(APDUException::class)
    fun selectApplicationByFID(dispatcher: Dispatcher?, slotHandle: ByteArray?, fileID: ByteArray): CardResponseAPDU? {
     ...
     */
    @Throws(APDUException::class)
    fun selectApplicationByAID(dispatcher: Dispatcher?, slotHandle: ByteArray?, aid: ByteArray?): CardResponseAPDU? {
    ...
    fun readFile(
        ...
        try {
            ...
            do {
                if (!isRecord) {
                    val readBinary: CardCommandAPDU = if (shortEf != null) {
                        if (numRead > 0xFF) {
                    ...
                    response = readBinary.transmit(
                        dispatcher, slotHandle, CardCommandStatus.response(
                            0x9000, 0x6282,
                            0x6A84, 0x6A83, 0x6A86, 0x6B00
                        )
                    )
                } else {
                    val readRecord: CardCommandAPDU = if (shortEf != null) {
                        ReadRecord(shortEf, i)
                    } else {
                        ReadRecord(i)
                    }
                    response = readRecord.transmit(
                        dispatcher, slotHandle, CardCommandStatus.response(
                            0x9000, 0x6282,
                            0x6A84, 0x6A83
                        )
                ...
                if (!trailer.contentEquals(byteArrayOf(0x6A.toByte(), 0x84.toByte())) && !trailer.contentEquals(
     ...
     */
    @Throws(APDUException::class)
    fun selectReadFile(dispatcher: Dispatcher, slotHandle: ByteArray?, fileID: ByteArray): ByteArray {
        var fcp: FCP? = null
    ...
    private fun isRecordEF(fcp: FCP?): Boolean {
        if (fcp == null) {
            ...
            return if (dataElements!!.isLinear || dataElements.isCyclic) {
                true
            } else {
                false
            }
    ...
    @JvmStatic
    fun isShortEFIdentifier(fileID: ByteArray): Boolean {
        return fileID.size == 1
    }

    @Throws(APDUException::class)
    fun writeFile(dispatcher: Dispatcher?, slotHandle: ByteArray?, fileID: ByteArray, data: ByteArray) {
    ...
    @Throws(APDUException::class)
    private fun writeFile(fcp: FCP, dispatcher: Dispatcher, slotHandle: ByteArray, data: ByteArray) {
        if (isRecordEF(fcp)) {
