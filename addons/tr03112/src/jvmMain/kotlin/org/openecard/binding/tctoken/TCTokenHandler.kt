 ...
 */
class TCTokenHandler(ctx: Context) {
    ...
    init {
        ...
        schemaValidator = FuturePromise<DocumentSchemaValidator?>(Callable {
    ...
    private fun processBinding(ctx: Context?, tokenReq: TCTokenRequest): TCTokenResponse {
        val token = tokenReq.getTCToken()
        try {
    ...
    fun handleActivate(params: MutableMap<String?, String?>?, ctx: Context?): BindingResult {
        var tokenReq: TCTokenRequest? = null
        try {
            tokenReq = TCTokenRequest.Companion.fetchTCToken(params)
    ...
    fun handleActivateInner(ctx: Context?, tokenReq: TCTokenRequest): TCTokenResponse {
        val token = tokenReq.getTCToken()
...
//	}
        try {
            ...
            var errorMsg = innerException!!.getLocalizedMessage()
            errorMsg = if (errorMsg == null) "" else errorMsg // fix NPE when null is returned instead of a message
            when (errorMsg) {
                "The target server failed to respond" -> errorMsg = LANG_TR.translationForKey(
                    ErrorTranslations.NO_RESPONSE_FROM_SERVER
                )

                ECardConstants.Minor.App.INT_ERROR + " ==> Unknown eCard exception occurred." -> errorMsg =
                    LANG_TR.translationForKey(
                        ErrorTranslations.UNKNOWN_ECARD_ERROR
                    )

                "Internal TLS error, this could be an attack" -> errorMsg =
            ...
            val paosAdditionalMinor = w.getAdditionalResultMinor()
            ...
            try {
                ...
                val msg2 = ("The RefreshAddress contained in the TCToken is invalid. Redirecting to the "
                ...
                response.addAuxResultData(
                    AuxDataKeys.REDIRECT_LOCATION, ex.getBindingResult().auxResultData.get(
    ...
    private fun createResponseFromWsEx(ex: WSHelper.WSException, response: TCTokenResponse): String? {
        val errorMsg: String?
        val minor = ex.resultMinor

        when (minor) {
            ECardConstants.Minor.Disp.TIMEOUT, ECardConstants.Minor.SAL.CANCELLATION_BY_USER, ECardConstants.Minor.IFD.CANCELLATION_BY_USER -> {
     ...
     */
    private fun createMessageFromUnknownError(@Nonnull w: PAOSException): String {
        var errorMsg = "\n"
    ...
    companion object {
        ...
        fun disconnectHandle(dispatcher: Dispatcher, connectionHandle: ConnectionHandleType?) {
        ...
        fun destroySession(dispatcher: Dispatcher, connectionHandle: ConnectionHandleType?) {
        ...
        private fun prepareForTask(request: TCTokenRequest, connectionHandle: ConnectionHandleType?) {
        ...
        private fun waitForTask(task: Future<*>) {
            try {
                ...
                if (ex.cause is PAOSException) {
                    throw ex.cause as PAOSException?
                } else if (ex.cause is DispatcherException) {
        ...
        private fun determineRefreshURL(request: TCTokenRequest, response: TCTokenResponse): TCTokenResponse {
            try {
                val endpointStr = response.getRefreshAddress()
...
//	    }
                // determine redirect
                val resultPoints: MutableList<Pair<URL?, TlsServerCertificate?>> = ctx.certs
                ...
                val communicationErrorAddress = response.getTCToken().getComErrorAddressWithParams(code)
