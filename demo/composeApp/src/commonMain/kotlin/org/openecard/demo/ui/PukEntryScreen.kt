...
fun PukEntryScreen(
	...
	var isPukVisible by remember { mutableStateOf(false) }
	...
	Scaffold(
		...
		Column(
			...
			OutlinedTextField(
				...
				label = { Text("PUK") },
				visualTransformation = if (isPukVisible) VisualTransformation.None else PasswordVisualTransformation(),
				trailingIcon = {
					val icon = if (isPukVisible) Icons.Default.VisibilityOff else Icons.Default.Visibility
					val description = if (isPukVisible) "Hide password" else "Show password"
					IconButton(onClick = { isPukVisible = !isPukVisible }) {
